  ...
  import { setCosmetics, setCosmeticsCategories, setLatestCosmetics } from "./redux/slices/CosmeticsSlice";
  ...
  const DataLoader = () => {
    ...
    useEffect(() => {
      const getApiData = async () => {
        ...
        try {
          const [newsResponse, newcosmeticsResponse, cosmeticsResponse] = await Promise.all([
            getData<ApiResponse<NewsData>>(GetEndpoints("news")),
            getData<ApiResponse<CosmeticsData>>(GetEndpoints("newCosmetics")),
            getData<ApiResponse<CosmeticsData>>(GetEndpoints("latest")),
          ]);
